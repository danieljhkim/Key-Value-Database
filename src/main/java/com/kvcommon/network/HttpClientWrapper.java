 ...
 *
 * Map<String, String> payload = Map.of("key", "foo", "value", "bar");
 * client.post("http://localhost:9001/set", payload);
 *
 * String value = client.get("http://localhost:9001/get?key=foo", String.class);
...
public class HttpClientWrapper {
    ...
    private void validateResponse(HttpResponse<?> response) throws Exception {
        ...
        throw new RuntimeException("Request failed with status code: " + code + ", body: " + response.body());
    }
}